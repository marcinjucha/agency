...
- `regenerate-helpers` — re-inlines canonical helpers into all opt-in Code nodes. Opt-in is via paired markers (`// @inline <name>` … `// @end-inline <name>`) wrapping the helper body in the Code node. Recursively scans `workflows/` (Workflows/ + Marketplace/ + any future subdir). Adding a new shared helper = one entry in the INLINE_HELPERS map + one evaluator file. **Currently inlined helpers (7):**
  - `supabase-request` — Promise/`https`-based PostgREST client (~30 lines, used in 20+ nodes). Accepts optional `opts.prefer` to override the `Prefer` header (default `return=representation`; pass `return=minimal` for write-and-forget)
  - `supabase-crud` — convenience wrappers `supabaseGet(table, filterKey, value, select?)` / `supabaseGet(table, queryParams)` and `supabasePatch(path, body, opts?)` over `supabaseRequest`. Used by Marketplace workflows + Orchestrator's Handle Skipped and Failed + Update Response Handler. Depends on `supabase-request` being in scope (a node opting into `supabase-crud` MUST also have a `supabase-request` marker pair, plus `env-supabase-preamble`)
