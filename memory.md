...
- **CMS loaders use `ensureQueryData` (blocking), NOT `prefetchQuery`** — `prefetchQuery` causes hydration mismatch: server renders loading state, client has data → React hydration error. `ensureQueryData` blocks loader until data ready → server and client render identical HTML. Pattern: `ensureQueryData` in loader + `useSuspenseQuery` in component. Corrected twice: first rule said prefetchQuery, second correction (2026-04-16) established ensureQueryData as correct.
...
- **`(input: unknown)` in TanStack Start `inputValidator` breaks RPC** — Using `(input: unknown)` as validator function signature causes TanStack Start RPC to fail silently. Must use typed `z.infer<typeof schema>` or pass Zod schema directly to `inputValidator`. WHY: RPC serialization layer needs schema metadata, not opaque unknown type.
- **Browser client (`createClient()`) fails during SSR — no cookies in server context** — `createClient()` (browser Supabase) returns empty data during SSR because server has no browser cookies. All data fetching in loaders AND component queryFn must use server fns (`createServerClient()` with cookie forwarding). WHY: TanStack Start SSR runs on server where `document.cookie` is unavailable.
