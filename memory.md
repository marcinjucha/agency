...
- **Supabase `.select('id')` returns ONLY requested columns — test mocks mask this** — `.select('id')` on Supabase query returns `{ id }` only, not full row. Vitest mocks that return full objects regardless of `.select()` hide this bug. Tests pass but runtime breaks when code accesses columns not in the select list. Always match mock return shape to actual `.select()` columns. (2026-04-14)
- **`step_type` lives on `workflow_steps`, NOT `workflow_step_executions`** — Execution table stores execution state (status, result, error). Step metadata (step_type, config, position) is on workflow_steps. Joining executions to steps required for any query needing step_type. (2026-04-14)
- **NULL `survey_link.workflow_id` = no workflow fires (breaking change from "all matching")** — After per-link workflow binding, links without explicit workflow_id assignment silently skip workflow triggering. Previously all matching workflows fired for any trigger. Migration requires backfilling workflow_id on existing survey_links. (2026-04-14)
