...
- **Adding new workflow step type requires 14 files across CMS + n8n** — Audit found: CMS (config panel, node component, output schema, icon, 6 registry entries) + n8n (handler subworkflow, Orchestrator Switch case, Flatten mapping). 10 of 14 registries use `Record<string>` not `Record<StepType>` — typos in step type keys fail silently. (2026-04-13)
...
- **engine/utils.ts duplicates output schemas from messages.ts** — DRY violation root cause: messages.ts imports break vitest (server-only module in test context). Output schemas were copy-pasted as workaround. Fix requires extracting shared schemas to a test-safe module. (2026-04-13)
...
- **companyName declared in trigger-schemas.ts but never produced by n8n** — All 4 trigger types (survey_submitted, booking_confirmed, lead_scored, manual) declare `companyName` variable in CMS registry, but NO n8n Trigger Handler subworkflow actually produces it. Users get literal `{{companyName}}` in emails — silent bug, no error in logs. (2026-04-13)
- **Stale comment in workflow-templates.ts claims triggers are NOT steps** — Lines 11-13 say "Triggers are NOT workflow_steps" but since AAA-T-153 triggers ARE real workflow_steps executed by Orchestrator. Stale comments mislead AI agents into wrong architecture assumptions. (2026-04-13)
...
- **ALL user-facing Polish strings MUST go through messages.ts** — User caught hardcoded Polish in step-registry.ts ("dlaczego tekst nie jest w messages?"). Applies to ALL files including registry/config — not just components. No exceptions for "internal" files if strings reach UI. (2026-04-13, AAA-T-190)
...
## Bugs Found (AAA-T-190 session)

- **`export { X } from './module'` doesn't create local binding in same file** — Re-export syntax only forwards to external consumers. If same file needs to USE `X`, must have separate `import { X } from './module'`. Caused runtime errors during step-registry refactor when barrel re-exports were added alongside local usage. (2026-04-13, AAA-T-190)
- **step-registry.ts had hardcoded Polish labels instead of messages.ts keys** — i18n violation found during plugin architecture refactor. Registry used literal strings ("Wyslij email") instead of message keys ("steps.send_email.label"). Silent — no error, just untranslatable UI. (2026-04-13, AAA-T-190)
- **TRIGGER_SUBTYPE_CONFIGS missing 'scheduled' key — only caught by Record<StepType>** — Changing from `Record<string>` to `Record<StepType>` immediately surfaced a missing key that had been silently absent. Validates architecture decision to type-narrow all registries. (2026-04-13, AAA-T-190)
- **Narrow function signatures break callback usage (`getStepTypeLabel(type: StepType)` vs `(string) => string`)** — Typed as `StepType` param but `collectAvailableVariables` passes plain `string`. TS2345 at compile. Widen to `string` with internal guard when function is used as callback. (2026-04-13, AAA-T-190)

## Architecture Decisions (AAA-T-190 session)
...
- **node-registry.ts still duplicates step-registry.ts data** — borderColor, icon, description, label duplicated in node-registry.ts despite step-registry centralization. "14 files for new step type" reduced to ~10-11, not 4. Incomplete centralization — tech debt for next refactor. (2026-04-13, AAA-T-190)
- **`description` field in step-registry.ts bypasses messages.ts (same as label bug)** — label was fixed to use `labelKey` + bridge pattern, but `description` still uses hardcoded Polish strings. Known i18n debt. (2026-04-13, AAA-T-190)
