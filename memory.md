...
- **companyName declared in trigger-schemas.ts but never produced by n8n** — All 3 trigger types (survey_submitted, booking_confirmed, lead_scored) declare `companyName` variable in CMS registry, but NO n8n Trigger Handler subworkflow actually produces it. Users get literal `{{companyName}}` in emails — silent bug, no error in logs. (2026-04-13)
