...
| **workflow-engine**      | `.claude/skills/ag-workflow-engine/SKILL.md`   | Workflow execution engine — n8n Orchestrator dispatch, $getWorkflowStaticData state management, condition branching (literal fallback), variable context (trigger hydration), SSRF protection, service role client, test mode, adding new step/trigger types |
