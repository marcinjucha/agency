...
function StatsGrid({
  ...
  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <StatCard label={messages.dashboard.totalSurveys} value={stats?.surveys ?? '—'} loading={isLoading} />
      <StatCard label={messages.dashboard.totalResponses} value={stats?.responses ?? '—'} loading={isLoading} />
      <StatCard label={messages.dashboard.appointments} value={stats?.appointments ?? '—'} loading={isLoading} />
      {isSuperAdmin && (
        <StatCard
