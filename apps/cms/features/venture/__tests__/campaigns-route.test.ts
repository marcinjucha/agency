...
import { Route } from '../../../app/routes/api/venture/campaigns/$slug'
...
describe('GET /api/venture/campaigns/$slug — route handler', () => {
  it('200 with the contract shape + edge-cache + CORS headers when the campaign exists', async () => {
    ...
    const res = await get('https://cms/api/venture/campaigns/kacper')

    expect(res.status).toBe(200)
    expect(mockRead).toHaveBeenCalledWith('kacper')
  ...
  it('404 with {error:"not_found"} when the read returns ok(null) — missing OR unpublished', async () => {
    ...
    const res = await get('https://cms/api/venture/campaigns/ghost')
  ...
  it('404 when the slug is empty (trailing slash / no segment) — never calls the read layer', async () => {
...
describe('OPTIONS /api/venture/campaigns/$slug — CORS preflight', () => {
