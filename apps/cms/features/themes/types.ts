 ...
 * N klientów · M kampanii". Both counts are real: `clients` from
 * `so_clients.theme_id`, `campaigns` from `so_campaigns.theme_id`.
 */
export type ThemeUsage = { clients: number; campaigns: number }
 ...
 * `so_clients.theme_id`, `tenants.theme_id`, OR `so_campaigns.theme_id` still
 * points at it (the FK is `ON DELETE SET NULL`, so a delete would silently
 * un-assign live rows). Distinct from `ThemeUsage` — the guard also counts the
 * tenant (org default) reference, which the library "used by" line does not
 * surface.
 */
export type ThemeReferences = { clients: number; tenants: number; campaigns: number }
