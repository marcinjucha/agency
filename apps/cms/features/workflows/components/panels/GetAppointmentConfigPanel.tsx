...
import { getAppointmentConfigSchema } from '../../validation'
import { STEP_MAP } from '../../step-registry'
import { resolveOutputSchema } from '../../utils/step-labels'
import type { StepConfigGetAppointment } from '../../types'
import type { ConfigPanelProps } from './index'

type GetAppointmentFormData = StepConfigGetAppointment

const outputFields = resolveOutputSchema(STEP_MAP['get_appointment'].outputSchema)

export function GetAppointmentConfigPanel({ stepConfig, onChange, availableVariables, isInvalid }: ConfigPanelProps) {
  ...
  const config = stepConfig as GetAppointmentFormData | undefined
  ...
  const {
    ...
    trigger,
    formState: { errors },
  } = useForm<GetAppointmentFormData>({
    mode: 'onChange',
    resolver: zodResolver(getAppointmentConfigSchema),
    defaultValues: {
      type: 'get_appointment',
      appointmentIdExpression: config?.appointmentIdExpression ?? '{{appointmentId}}',
    },
  })
  ...
  return (
    <div className="space-y-5">
      <p className="text-sm text-foreground">
        {m.getAppointmentDescription}
      </p>

      {/* appointmentIdExpression */}
      <div className="space-y-1.5">
        <Label htmlFor="get-appointment-id-expr" className="text-sm font-medium">
          {m.getAppointmentIdExpressionLabel}
        </Label>
        <p className="text-xs text-muted-foreground">
          {m.getAppointmentIdExpressionHint}
        </p>
        <div className="flex items-center gap-1">
          <Controller
            name="appointmentIdExpression"
            control={control}
            render={({ field: f }) => (
              <Input
                id="get-appointment-id-expr"
                ref={(el) => {
                  expressionInputRef.current = el
                  f.ref(el)
                }}
                placeholder={m.getAppointmentIdExpressionPlaceholder}
                value={f.value ?? ''}
                onChange={f.onChange}
                onBlur={f.onBlur}
                className="h-9 font-mono text-xs"
                aria-label={m.getAppointmentIdExpressionLabel}
                aria-invalid={!!errors.appointmentIdExpression}
                aria-describedby={errors.appointmentIdExpression ? 'get-appointment-id-expr-error' : undefined}
              />
            )}
          />
          {variables.length > 0 && (
            <VariableInserter
              ...
              onChange={(value) =>
                setValue('appointmentIdExpression', value, { shouldDirty: true })
              }
              currentValue={formValues.appointmentIdExpression ?? ''}
            />
          )}
        </div>
        {errors.appointmentIdExpression && (
          <p id="get-appointment-id-expr-error" role="alert" className="text-xs text-destructive">
            {errors.appointmentIdExpression.message}
          </p>
        )}
      </div>
