...
│       ├── index.tsx        # Homepage (hardcoded landing)
...
> Note: the homepage landing is now **hardcoded** (`features/marketing/content.ts` + components),
> not CMS-driven. The only dynamic value is the survey CTA URL, fetched via
> `getLandingCtaUrlFn` (reads `landing_pages.cta_url`) and threaded to the CTAs.
